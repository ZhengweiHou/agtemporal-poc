...
func TestBatchCaseV2(t *testing.T) {
	...
	require.NoError(t, err)
	hzwstepDef, _ := b.BuildTasklet(stephzw, batch.WithActivityName("v2-stephzw"))

	// ═══  phase 声明定义
	phase_validat := batch.NewActivityPhase("校验", validateDef, getInFilePath)
	phase_wf_validat := batch.NewFlowPhase("step1-校验文件", phase_validat) // 包装为flow

	phase_hzwstep := batch.NewActivityPhase("phase_hzwstep", hzwstepDef, getInFilePath)
	phase_wf_hzwstep := batch.NewFlowPhase("phase_wf_hzwstep", phase_hzwstep)                     // 包装为flow
	phase_shard := batch.NewShardPhase("step2a-分片处理", &shardPartitioner{}, engineDef, getInShard) // P2a: 分片 Child WF
	phase_sum := batch.NewActivityPhase("step2b-金额汇总", sumDef, getInFilePath)                     // P2b

	phase_para_p1 := batch.Parallel(
		phase_shard,
		phase_sum,
		// phase_hzwstep,
		phase_wf_hzwstep,
	) //将两个节点包装到一个并行节点
	// phase_para_p1 := batch.Parallel(phase_wf_validat, phase_shard, phase_sum)                     //将两个节点包装到一个并行节点

	phase_report := batch.NewActivityPhase("step3-打印结果", reportDef, getInReport) // P3
...
func stephzw(ctx context.Context, input batch.BatchInput) (batch.BatchResult, error) {
	pjson, _ := json.Marshal(input.Params)
	slog.Info(fmt.Sprintf("===stephzw: %s", pjson))
